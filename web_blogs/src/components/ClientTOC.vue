...
const displayText = computed(() => {
  return function (tocItem: TocItem) {
    return getDisplayText(tocItem.text)
  }
})

const safeId = computed(() => {
  return function (tocItem: TocItem) {
    return getSafeId(tocItem.text)
  }
})
...
const scrollToHeading = (e, id: string) => {
  // 🌟 防止任何超链接的意外默认行为
  e.preventDefault();
...
<template>
  <div
      class="bg-white/60 dark:bg-slate-800/50 backdrop-blur-xl rounded-3xl p-6 border border-white/40 dark:border-white/10 shadow-xl sticky top-28 transition-colors duration-700 max-h-[75vh] overflow-y-auto custom-scrollbar">
    <h3 class="font-black text-slate-900 dark:text-white mb-4 border-l-4 border-indigo-500 pl-2 text-sm uppercase tracking-widest">
      Table of Contents
    </h3>
    <nav class="flex flex-col gap-2 relative">
      <div class="absolute left-0 top-0 bottom-0 w-[2px] bg-slate-200 dark:bg-slate-700/50 rounded-full"></div>
      <button
          v-for="(item,index) in props.toc"
          :key="index"
          :class="`text-left text-sm transition-all duration-300 line-clamp-2 cursor-pointer relative pl-4
      ...
      `"
          @click="(e) => scrollToHeading(e, safeId(item))"
      >
        <span v-if="safeId(item) === activeId"
              class="absolute left-[-5px] top-[50%] -translate-y-[50%] w-[6px] h-[6px] rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.8)]"></span>
